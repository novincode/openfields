 ...
 */
function cofld_render_post_object_field( $field, $value, $field_id, $field_name, $settings ) {
	...
	<div class="cofld-post-object-field" <?php echo $data_string; ?>>
		<?php if ( $multiple ) : ?>
		...
		<div class="cofld-selected-items">
			<?php foreach ( $selected_posts as $post ) : ?>
				<div class="cofld-selected-item" data-id="<?php echo esc_attr( $post['id'] ); ?>">
					<span class="cofld-item-title"><?php echo esc_html( $post['title'] ); ?></span>
					<span class="cofld-item-type"><?php echo esc_html( $post['type'] ); ?></span>
					<button type="button" class="cofld-remove-item" title="<?php esc_attr_e( 'Remove', 'codeideal-open-fields' ); ?>">
		...
		<!-- Search input -->
		<div class="cofld-search-container">
			<input 
				type="text" 
				class="cofld-search-input" 
				placeholder="<?php esc_attr_e( 'Search...', 'codeideal-open-fields' ); ?>"
				autocomplete="off"
			/>
			<div class="cofld-search-results"></div>
		</div>
	</div>
	<?php
}
...
add_action( 'cofld_render_field_post_object', 'cofld_render_post_object_field', 10, 5 );
