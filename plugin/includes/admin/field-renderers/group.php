 ...
 */
function openfields_render_group_field( $field, $value, $field_id, $base_name, $settings, $object_id, $object_type = 'post' ) {
	global $wpdb;
	...
	if ( empty( $sub_fields ) ) {
		echo '<p class="description">' . esc_html__( 'No sub-fields configured for this group.', 'openfields' ) . '</p>';
		return;
	}
	...
	?>
	<div 
		class="openfields-group"
		id="<?php echo esc_attr( $unique_id ); ?>"
		data-name="<?php echo esc_attr( $base_name ); ?>"
		data-layout="<?php echo esc_attr( $layout ); ?>"
	>
		<div class="openfields-group-content" data-layout="<?php echo esc_attr( $layout ); ?>">
			<?php
			foreach ( $sub_fields as $sub_field ) {
				openfields_render_group_subfield( $sub_field, $base_name, $object_id, $object_type );
			}
			?>
		</div>
	</div>
...
function openfields_render_group_subfield( $sub_field, $parent_name, $object_id, $object_type = 'post' ) {
	...
	>
		<?php if ( ! empty( $sub_field->label ) ) : ?>
			<div class="openfields-field-label">
				<label for="<?php echo esc_attr( $sub_meta_key ); ?>">
					...
					<?php if ( ! empty( $sub_settings['required'] ) || ! empty( $sub_field->required ) ) : ?>
						<span class="openfields-field-required" aria-label="required">*</span>
					<?php endif; ?>
				</label>
			</div>
		<?php endif; ?>

		<div class="openfields-field-input">
			<?php
			openfields_render_group_subfield_input( $sub_field, $sub_value, $sub_meta_key, $sub_meta_key, $sub_settings, $object_id, $object_type, $parent_name );
			?>
		</div>

		<?php if ( ! empty( $sub_settings['instructions'] ) || ! empty( $sub_field->instructions ) ) : ?>
			<p class="openfields-field-description">
